...
google.bookmarkbubble.Bubble.prototype.NUMBER_OF_TIMES_TO_DISMISS=10;
	...
	function isBookmarked() {
		if (typeof(Storage) !== "undefined") {
			if (sessionStorage.bookmark) {
				if(Number(sessionStorage.bookmark) == 0 ){
				return 0;
				}
				else {
				return 1;
			...
			/*If failed then via Cookies*/
		}
	};
	if(!isBookmarked())
	{
		if(bubble.showIfAllowed()) {
			sessionStorage.bookmark = 1;
		}
		else{
			sessionStorage.bookmark = 0;
		}
